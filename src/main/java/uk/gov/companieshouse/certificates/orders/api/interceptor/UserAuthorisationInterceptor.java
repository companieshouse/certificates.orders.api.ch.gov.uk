...
public class UserAuthorisationInterceptor extends HandlerInterceptorAdapter {
    ...
    private boolean validateOAuth2(HttpServletRequest request, HttpServletResponse response) {
        if (!POST.matches(request.getMethod())) {
            ...
            Map<String, Object> logMap = new HashMap<>();
            logMap.put(LoggingConstants.CERTIFICATE_ID_LOG_KEY, certificateId);
            logMap.put(LoggingConstants.REQUEST_ID_LOG_KEY, request.getHeader(LoggingConstants.REQUEST_ID_HEADER_NAME));
            logMap.put(LoggingConstants.IDENTITY_LOG_KEY, identity);
            ...
            if (item.isPresent()) {
                ...
                logMap.put(LoggingConstants.USER_ID_LOG_KEY, userId);
                ...
                logMap.put(LoggingConstants.STATUS_LOG_KEY,HttpStatus.NOT_FOUND);
